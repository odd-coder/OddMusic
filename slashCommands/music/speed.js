...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(args[0] === undefined || isNaN(args[0]) || Number(args[0]) < 50 || Number(args[0]) > 300) return interaction.reply({ ephemeral: true, content: `<:oddno:968555009908293652> **No __valid__ Bassboost-Level between 50 and 300 % provided!** (100 % == normal speed)\n Usage: \`${prefix}speed 125\``}).catch(() => null);
        ...
        return interaction.reply({ ephemeral: false, content: `<:oddmu:968552025723928647> **Successfully changed the Speed to \`${Math.floor(speed) / 100}x\` of the Original Speed (${speed}%)**`}).catch(() => null);
    },
};