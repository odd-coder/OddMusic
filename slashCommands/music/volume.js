...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        ...
        if(!args[0] || isNaN(args[0]) || Number(args[0]) < 1 || Number(args[0]) > 150) return interaction.reply({ ephemeral: true, content: `<:oddno:968555009908293652> **No __valid__ Volume between 1 and 100 % provided!** Usage: \`${prefix}volume 25\``}).catch(() => null);
        const volume = Number(args[0]);
        queue.volume = volume;
        oldConnection.state.subscription.player.state.resource.volume.setVolume(volume / 100);
        
        return interaction.reply({ ephemeral: false, content: `<:oddv:968567000307736607> **Successfully changed the Volume to \`${volume}%\`**`}).catch(() => null);
    },
};