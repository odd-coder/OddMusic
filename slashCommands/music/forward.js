...
module.exports = {
    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            if(!args[0] || isNaN(args[0])) return interaction.reply({ ephemeral: true, content:`<:oddno:968555009908293652> **You forgot to add the forwarding-time!** Usage: \`${prefix}forward <Time-In-S>\``}).catch(() => null);
            
            if(Number(args[0]) < 0 || Number(args[0]) > Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1))
            return interaction.reply({ ephemeral: true, content:`<:oddno:968555009908293652> **The Forward-Number-Pos must be between \`0\` and \`${Math.floor((queue.tracks[0].duration - curPos) / 1000 - 1)}\`!**`}).catch(() => null);
            ...
            interaction.reply({ ephemeral: false, content:`<:oddmu:968552025723928647> **Forwarded for \`${args[0]}s\` to \`${client.formatDuration(newPos)}\`**!`}).catch(() => null);
        } catch(e) { 
            console.error(e);
            interaction.reply({ ephemeral: true, content:`<:oddno:968555009908293652> Could not join your VC because: \`\`\`${e.interaction || e}`.substring(0, 1950) + `\`\`\``}).catch(() => null);
        }
    },
};