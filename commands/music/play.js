...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if(!args[0]) return message.reply(`<:oddno:968555009908293652> Please add the wished Music via: \`${prefix}play <Name/Link>\``).catch(() => null);
        ...
        if(!oldConnection) {
            ...
            try {
                ...
                return message.reply(`<:oddno:968555009908293652> Could not join the VC because: \`\`\`${e.message || e}`.substr(0, 1950) + `\`\`\``).catch(() => null);
            }
        }
        try {
            ...
            if(!playlist) {
                ...
                if(!queue || queue.tracks.length == 0) { 
                    ...
                    return m.edit(`<:oddmu:968552025723928647> **Now Playing: __${song.title}__** - \`${song.durationFormatted}\``).catch(() => null);
                }
                // Add the song to the queue
                queue.tracks.push(client.createSong(song, message.author))
                // edit the loading message     
                return m.edit(`<:oddplus:968552913574178886> **Queued at \`${client.queuePos(queue.tracks.length - 1)}\`: __${song.title}__** - \`${song.durationFormatted}\``).catch(() => null);
            } 
            /* FOR PLAYLIST REQUEST */
                ...
                if(!queue || queue.tracks.length == 0) { 
                    ...
                    await client.playSong(message.member.voice.channel, song);
                    // edit the loading message     
                    return m.edit(`<:oddmu:968552025723928647> **Now Playing: __${song.title}__** - \`${song.durationFormatted}\`\n> **Added \`${playlist.videos.length - 1} Songs\` from the Playlist:**\n> __**${playlist.title}**__`).catch(() => null);
                }
                // Add the playlist songs to the queue
                playlist.videos.forEach(song => queue.tracks.push(client.createSong(song, message.author)))
                // edit the loading message                    
                return m.edit(`<:oddplus:968552913574178886> **Queued at \`${client.queuePos(queue.tracks.length - (playlist.videos.length - 1))}\`: __${song.title}__** - \`${song.durationFormatted}\`\n> **Added \`${playlist.videos.length - 1} Songs\` from the Playlist:**\n> __**${playlist.title}**__`).catch(() => null);
            }

        } catch (e){ console.error(e);
            return message.reply(`<:oddno:968555009908293652> Could not play the Song because: \`\`\`${e.message || e}`.substr(0, 1950) + `\`\`\``).catch(() => null);
        }
    },
};
